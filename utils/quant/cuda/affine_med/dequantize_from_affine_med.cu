#include "dequantize_from_affine_med.cuh"
...
__global__ void dequantize_from_affine_med_kernel(
    ...
    const int scale_word_index = super_block * metadata_words_per_super;
    const int min_word_index = super_block * metadata_words_per_super;
    const int packed_word_index =
        (super_block * sub_blocks_per_super + sub_block) * kPackedWordsPerWeightSubBlock;

    uint32_t weight_words[kPackedWordsPerWeightSubBlock];
    weight_words[0] = static_cast<uint32_t>(packed[packed_word_index]);
    weight_words[1] = static_cast<uint32_t>(packed[packed_word_index + 1]);
    weight_words[2] = static_cast<uint32_t>(packed[packed_word_index + 2]);
    weight_words[3] = static_cast<uint32_t>(packed[packed_word_index + 3]);

    const int scale_code = load_unsigned_bits(
        reinterpret_cast<const uint32_t*>(sub_scales + scale_word_index),
        sub_block,
        kScaleBits
    );
    const int min_code = load_signed_bits(
        reinterpret_cast<const uint32_t*>(sub_mins + min_word_index),
        sub_block,
        kMinBits
    );
    ...
    for (int value_index = 0; value_index < kSubBlockSize; value_index += 2) {
        ...
        const float lo_q = static_cast<float>(load_unsigned_bits(weight_words, value_index, kWeightBits));
        const __half lo = __float2half((lo_q * scale) + min_value);

        if (out_index + 1 < original_numel) {
            const float hi_q = static_cast<float>(load_unsigned_bits(weight_words, value_index + 1, kWeightBits));
...
void launch_dequantize_from_affine_med(
    ...
    dequantize_from_affine_med_kernel<<<blocks, kThreadsPerBlock, 0, stream>>>(
        packed,
        sub_scales,
        sub_mins,
        super_scales,
