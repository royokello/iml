...
void validate_inputs(
    ...
    if (super_block_size % iml::cuda::dequantize_from_affine_med::kSubBlockSize != 0) {
    ...
    const int64_t sub_blocks_per_super =
        super_block_size / iml::cuda::dequantize_from_affine_med::kSubBlockSize;
    const int64_t metadata_words_per_super = packed_words_for_values(
        sub_blocks_per_super,
        iml::cuda::dequantize_from_affine_med::kScaleBits
    );
    const int64_t expected_metadata_words = num_super_blocks * metadata_words_per_super;
    const int64_t expected_packed_words =
        num_super_blocks *
        sub_blocks_per_super *
        iml::cuda::dequantize_from_affine_med::kPackedWordsPerWeightSubBlock;

    if (packed.numel() < expected_packed_words) {
        throw std::invalid_argument("packed does not contain enough words");
    }
    if (sub_scales.numel() < expected_metadata_words) {
        throw std::invalid_argument("sub_scales does not contain enough values");
    }
    if (sub_mins.numel() < expected_metadata_words) {
...
void dequantize_from_affine_med_fp16(
    torch::Tensor packed,
    ...
    iml::cuda::dequantize_from_affine_med::launch_dequantize_from_affine_med(
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def(
        "dequantize_from_affine_med_fp16",
        &dequantize_from_affine_med_fp16,
