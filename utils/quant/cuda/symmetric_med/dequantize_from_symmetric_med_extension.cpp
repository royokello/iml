...
void validate_inputs(
    ...
    if (super_block_size % iml::cuda::dequantize_from_symmetric_med::kSubBlockSize != 0) {
    ...
    if (packed.numel() % iml::cuda::dequantize_from_symmetric_med::kPackedWordsPerSubBlock != 0) {
        throw std::invalid_argument("packed must contain 3 int32 words per sub-block");
    }

    const int64_t sub_blocks_per_super =
        super_block_size / iml::cuda::dequantize_from_symmetric_med::kSubBlockSize;
    ...
    const int64_t expected_packed_words =
        expected_sub_metadata * iml::cuda::dequantize_from_symmetric_med::kPackedWordsPerSubBlock;
...
void dequantize_from_symmetric_med_fp16(
    torch::Tensor packed,
    ...
    iml::cuda::dequantize_from_symmetric_med::launch_dequantize_from_symmetric_med(
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def(
        "dequantize_from_symmetric_med_fp16",
        &dequantize_from_symmetric_med_fp16,
        "Dequantize packed signed int6 symmetric-med values into fp16"
    );
}
