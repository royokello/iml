...
@lru_cache(maxsize=1)
def _load_prebuilt_symmetric_med_dequant_module():
    module_name = "dequantize_from_symmetric_med_cuda"
    ...
    raise ModuleNotFoundError(
        "Prebuilt module 'dequantize_from_symmetric_med_cuda' not found. Build it first with "
        "'python setup.py build_ext --inplace' in utils/quant/cuda/symmetric_med."
    )


def dequantize_from_symmetric_med(
    ...
    module = _load_prebuilt_symmetric_med_dequant_module()
    module.dequantize_from_symmetric_med_fp16(
        qweight.contiguous(),
...
__all__ = ["dequantize_from_symmetric_med"]
