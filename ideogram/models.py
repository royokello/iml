...
class Ideogram4Transformer(nn.Module):
  ...
  def enable_block_offload(self, device: torch.device) -> None:
    self._offload_device = device

  def _move_non_layers_to(self, device: torch.device) -> None:
  ...
  def forward(
    ...
    *,
    ...
    if self._offload_device is not None:
      self._move_non_layers_to(self._offload_device)
    ...
    for layer in self.layers:
      if self._offload_device is not None:
        layer.to(self._offload_device, non_blocking=True)
    ...
    if self._offload_device is not None:
      self._move_non_layers_to(torch.device("cpu"))
