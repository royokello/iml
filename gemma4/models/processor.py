...
from transformers.utils import auto_docstring, is_vision_available, logging
...
if is_vision_available():
    try:
        from .vision.image_processor import Gemma4ImageProcessorKwargs, get_aspect_ratio_preserving_size
    except ModuleNotFoundError:
        class Gemma4ImageProcessorKwargs(TypedDict, total=False):
            do_convert_rgb: bool
            patch_size: int
            max_soft_tokens: int
            pooling_kernel_size: int

        def get_aspect_ratio_preserving_size(*args, **kwargs):
            raise ModuleNotFoundError(
                "Gemma4 image processing requires a newer transformers vision backend. "
                "Text-only usage can continue, but image inputs are unavailable in this environment."
            )
else:
    class Gemma4ImageProcessorKwargs(TypedDict, total=False):
        do_convert_rgb: bool
