...
def _quantize_denoiser(
    ...
    output_dir = Path(root) / "anima" / "model" / "denoiser" / variant
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Quantize the Anima denoiser.")
    ...
    parser.add_argument("--variant", default="base", help="Model variant subfolder (default: base).")
    parser.add_argument("--method", default="sym-high", help="Quantization method (default: sym-high).")
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    output_path = _quantize_denoiser(args.root, args.input, method=args.method, variant=args.variant)
